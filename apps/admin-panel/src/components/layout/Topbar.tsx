  ...
  return (
    <header className="flex items-center justify-between gap-4 border-b cmd-border cmd-surface px-6 py-3">
      ...
      <div className="flex shrink-0 items-center gap-3">
        ...
        <button
          type="button"
          aria-label="Open SOS center"
          onClick={() => router.push("/sos")}
          className="relative flex h-9 w-9 items-center justify-center rounded-lg border cmd-border cmd-muted transition hover:cmd-ember"
        >
        ...
        <div className="flex items-center gap-2 border-s cmd-border ps-3">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-xs capitalize cmd-muted">
              {admin?.role ?? "admin"}
            </p>
          </div>
        </div>
