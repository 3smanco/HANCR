...
- **التشخيص (مؤكَّد من الكود):** التنقّل مدفوع بالحالة عبر `redirect` في `app.dart` (`OrderActive→/tracking`…). `pickup_confirmation_screen.dart` كانت مبنية لكن **غير مدمجة** (مسار `/preview/pickup` فقط، ثيم HancrColors قديم)، و`_requestRide()` كان يتخطّاها للطلب مباشرة. حالة `arrived` مجرّد سطر نص — لا عدّاد انتظار.
- **✅ Phase A (backend reverseGeocode):** أُضيف `reverseGeocode(lat,lng)` لـ`places.service.ts` (+resolver) عبر Geocoding API legacy (يُفضّل route→neighborhood→sublocality) + `reverseGeocodeQuery` في `rider_gql.dart`. `tsc` rider-api = 0.
- **✅ Phase B1 (ضبط الالتقاط الدقيق):** شاشة جديدة `screens/booking/aurora_pickup_adjust_screen.dart` بهوية Aurora: خريطة كامل الشاشة zoom 17 + دبوس ember ثابت بالمركز + نبض + Tooltip باسم الشارع الحيّ (reverseGeocode debounced) + مناطق ركوب خضراء + Snap‑to مغناطيسي (هافرسين، عتبة 55م) + `AuroraButton.primary` «تأكيد الالتقاط». **دُمجت إجبارياً** في `_requestRide()` (الآن `async`؛ يلتقط Navigator+OrderBloc قبل await؛ يمرّر الإحداثيات المقفلة لـ`OrderCreateRequested`). نصوص جديدة في `app_localization.dart` (ar+en). `flutter analyze` (الملفات الـ4) = **No issues found**.
- **✅ Phase B2 (وصول السائق + عدّاد الانتظار):**
  - **Backend (مُتحقَّق tsc rider+driver=0، driver order.service.spec = 28/28):** عمود `arrived_at` (+migration `1782400000000-AddOrderArrivedAt`، auto‑globbed) يُضبط في `arrivedAtPickup`. رسوم الانتظار تُحسب في `finishRide` عبر `_computeWaitCharge` ((startTimestamp−arrivedAt)−`FREE_WAIT_SECONDS`=120، ⌈دقائق⌉×`service.perMinuteWait`) وتُطوى في `costBest/costAfterCoupon` قبل `_settlePayment` (العمولة تبقى على الأجرة الأساسية ⇒ رسوم الانتظار **كاملة للسائق**؛ additive ⇒ صفر للطلبات القديمة فلم تنكسر الاختبارات). أُظهرت `arrivedAt`+`waitCost`+`freeWaitSeconds` في `OrderType`+المابر+`order_gql`(activeOrder+orderUpdated)+`OrderModel`.
  - **Flutter (مُتحقَّق analyze=No issues):** ودجت جديدة `screens/tracking/widgets/aurora_arrived_panel.dart` (حزام «وصل سائقك» ember + عدّاد تنازلي أخضر للمهلة المجانية يتحوّل أحمر «بدأ احتساب وقت الانتظار» مع عدّ تصاعدي + **Vehicle Verification Pod**: لوحة بيضاء عالية التباين تُكبّر `plateNumber` + موديل/لون + أفاتار). مدمجة في `aurora_tracking_screen._buildBottomCard` عند `status==arrived` (بدل صف السائق العادي). **اهتزاز+صوت قسري** عبر `BlocListener<OrderBloc>` يرصد التحوّل لـ`arrived` (`Haptics.heavy()`+`SystemSound.alert`).
  - **🔑 إصلاح بنيوي مهم:** قناتا `ORDER_UPDATED` (راكب) و`DRIVER_ORDER_UPDATED` (سائق) **منفصلتان** ⇒ الراكب كان يستقبل تحوّلات السائق عبر FCM فقط، و`_onForegroundMessage` كان يعرض إشعاراً بلا تحديث. الآن `push_service.dart` يحمل `onOrderEvent` (+`onMessageOpenedApp`+`getInitialMessage` للفتح من الخلفية/الإغلاق)، و`app.dart` يربطه بـ`OrderActiveCheckRequested` ⇒ كل تحوّلات الدورة (assigned/arrived/started/completed) تتحدّث حيّاً في تطبيق الراكب.
- **🟡 Phase C (جزئي، مُتحقَّق analyze=No issues):** **C1 رادار البحث** — ودجت `screens/tracking/widgets/aurora_radar_beacon.dart` (حلقات ember نابضة متحدّة المركز) حلّت محل سبينر `_searchingDriverCard` (تخطيط رأسي + «نُبلغ السائقين القريبين»). **ربط رسوم B2 بالواجهة** — `_waitCostChip` في `_buildBottomCard` يعرض `order.waitCost` عند تراكمه. *C2 (بطاقة en‑route: سائق+ETA+اتصال/شات/مشاركة) و C3 (مسار حيّ + SOS) موجودان أصلاً في `aurora_tracking_screen`.* **متبقٍّ في C: C4 ورقة الدفع pre/post‑pay** (ميزة عميقة — تتطلّب تدفّق checkout البوابة/webhook؛ wallet/cash يُسوّى تلقائياً في finishRide).
- **🚀 نُشر حيّاً ومُتحقَّق (2026-06-27):** PR [#186](https://github.com/3smanco/HANCR/pull/186) مدموج في `main` (merge `b54b735`). **السيرفر:** `deploy-direct.sh` بـ`RUN_MIGRATIONS=1` (sudo -u info) → git pull + build rider/driver-api + migration **`AddOrderArrivedAt1782400000000` = [X] مُطبَّقة** + pm2 reload + health كلها ready. **APK الراكب:** بُني (arm64، ENV=production، مفتاح Maps الجديد `…I5RrQ` مُتحقَّق بـaapt في المانيفست، 50,911,987 بايت) → نُشر على `hancr.com/downloads/hancr-rider.apk` (**HTTP 200، الحجم مطابق**، نسخة احتياطية `.bak-20260627`). **تطبيق السائق لم يُعَد بناؤه** (تغييرات B2 من جهته backend فقط). **للمستخدم: ألغِ تثبيت APK القديم قبل الجديد.**
- **✅ Phase E (إلغاء + سبب + رسوم) — على فرع `feat/rider-journey-c-to-f`، مُتحقَّق (tsc rider=0، analyze=No issues):** عمودا `cancel_reason`+`cancellation_fee` (migration `1782500000000-AddOrderCancellation`). `cancelOrder(riderId, orderId, reason?)` يحسب الرسم (مجاني قبل إسناد السائق؛ بعده من `pricingRules.cancellationFee` أو `DEFAULT_CANCELLATION_FEE=5`) ويسجّل السبب — **يُسجَّل ويُعرض فقط؛ الخصم الفعلي من المحفظة follow-up مقصود**. resolver يقبل `reason`. أُظهر `cancellationFee`+`cancelReason` في OrderType/مابر/gql/OrderModel. Flutter: `cancel_reason_sheet.dart` (6 أسباب + تحذير رسم عند `status.hasDriver`) محلّ dialog `_confirmCancel` البسيط؛ `OrderCancelRequested(orderId, reason)` + bloc يمرّره.
- **✅ Phase F جزئي (Shield + أمزجة) — مُتحقَّق analyze=No issues:** شريط chips في بطاقة التتبع (الرحلة النشطة): «رحلتك مؤمّنة» (HANCR Shield) + هدوء/بدون موسيقى/رقم مخفي حسب `order.quietRide/audioOff/numberMasked`.
- **🚀 نُشر حيّاً ومُتحقَّق (2026-06-27، الدفعة الثانية):** PR [#187](https://github.com/3smanco/HANCR/pull/187) مدموج في `main` (merge `22aff25`). السيرفر: `deploy-direct.sh` RUN_MIGRATIONS=1 → **migration `AddOrderCancellation1782500000000` = [X]** (إلى جانب `AddOrderArrivedAt` [X]) + health كلها ready. APK الراكب أُعيد بناؤه (C1+E+F، arm64، production) → `hancr.com/downloads/hancr-rider.apk` (**HTTP 200، 50,978,027 بايت**، نسخة `.bak-20260627b`). **للمستخدم: ألغِ تثبيت القديم قبل الجديد.** الآن B1+B2+C1+E+F‑جزئي كلها حيّة.
- **✅ Phase D (تفصيل أجرة + تقييم سلبي) + C4-lite (بطاقة دفع) — frontend فقط، مُتحقَّق analyze=No issues** (commits 3b7eb66 + 604708f على الفرع، فوق merge #187):
...
- **الباكند:** `ssh 7bici@34.18.212.201` ثم **`sudo -u info`** (الريبو + pm2 مملوكان للمستخدم `info` لا `7bici` — درس مهم: استخدم `sudo -u info bash -lc '...'`) → `git pull` + `deploy-direct.sh` → كل الخدمات reloaded + health OK. **rider-api حيّ** (`api.hancr.com/rider/graphql` يرد `__typename`). يفعّل #1 (البحث legacy) و#2 (تخزين directions).
- **الـ APKs (بُنيت محلياً، arm64، ENV=production، مفتاح Maps الجديد):** الراكب 43,094,415 + السائق 40,590,508 بايت → رُفعت لـ `/var/www/hancr-landing/downloads/hancr-{rider,driver}.apk` (نسخ احتياطية `.bak-20260626b`، مملوكة www-data). **مُتحقَّق HTTP 200 على hancr.com/downloads.** الراكب يحمل #2، السائق يحمل #3.
- **الصلاحيات:** المالك أضاف يدوياً كتلة `permissions.allow` في `.claude/settings.json` (gh pr merge / ssh / scp / flutter build) — النشر المستقبلي لن يُحجب. *(الوكيل لا يستطيع تعديل صلاحياته بنفسه — حاجز أمني سليم.)*
- **للمستخدم:** ألغِ تثبيت التطبيق القديم قبل تثبيت الجديد.

## 🔎 ثلاث مشاكل بلاغ المالك (بحث الأماكن + خط المسار/السعر + طلب السائق) — تشخيص + إصلاحان (2026-06-26)
**البلاغ (3 لقطات):** (1) البحث عن مكان في خانة الوجهة لا يُظهر نتائج · (2) المسار يُرسم خطاً مستقيماً لا يراعي الطرق + السعر يبدو غير منطقي للمسافة · (3) تطبيق السائق: عند الطلب يصل إشعار FCM فقط، والضغط عليه لا يُظهر الطلب.
- **#1 البحث — السبب الجذري (مؤكَّد باختبار حيّ ضد Google):** الكود كان يستخدم **Places API (New)** وهي **معطَّلة** على المشروع 997014889761 (`SERVICE_DISABLED / PERMISSION_DENIED`) ⇒ `PlacesService.search` يعود `[]` بصمت. المُفعَّل فعلاً هو **Places API (legacy)** (اختبار Autocomplete رجّع «مطعم الكاتم، الدوحة» ✅) + Geocoding + Directions. **الإصلاح ✅:** أُعيدت كتابة `apps/rider-api/src/app/places/places.service.ts` لاستخدام legacy Autocomplete (+ Place Details عند الاختيار — العميل يدعم أصلاً جلب الإحداثيات عند الاختيار). `tsc --noEmit` rider-api = 0 errors. *(بديل مستقبلي: تفعيل "Places API (New)" من Cloud Console ثم العودة للـ endpoints الجديدة.)*
- **#3 طلب السائق — السبب الجذري (مؤكَّد):** الودجت `IncomingOrderSheet` كاملة لكنها **لم تُستدعَ من أي مكان**؛ شاشة `aurora_driver_home.dart` كانت تعالج `OrderActive` فقط ولا تعالج حالة `OrderIncoming` ⇒ الاشتراك `newOrderAvailable` يصل لكن لا شيء يعرضه (إشعار FCM منفصل فيظهر وحده). **الإصلاح ✅:** أُضيف `MultiBlocListener` في `aurora_driver_home.dart` يعرض `IncomingOrderSheet` كـmodal عند `OrderIncoming` (مع تمرير OrderBloc+AuthBloc صراحةً + حارس `_incomingShown`). `flutter analyze` = No issues found. *(تبقّى اختياري: ربط نقر إشعار FCM في الخلفية عبر `onMessageOpenedApp`/`getInitialMessage` في `push_service.dart`.)*
- **#2 خط المسار/السعر — السبب الجذري (مؤكَّد) — مُصلَح ✅ (بلا migration):** Directions API **تعمل** ⇒ المسافة/الأجرة **صحيحة وقائمة على الطريق**؛ المشكلة بصرية: شاشة التتبع كانت ترسم الخط من `order.points` (نقطتان) ⇒ خط مستقيم يجعل السعر يبدو غير منطقي. **الاكتشاف:** عمود `directions` (GeoPoint[]) موجود أصلاً في `OrderEntity` وغير مستخدَم ⇒ لا حاجة لـmigration. **الإصلاح:** (أ) `order.service.ts` يفكّ `route.polyline` عبر `decodePolyline()` الجديد ويخزّنه في `directions` عند `createOrder`؛ (ب) `OrderType` (rider dto) يصدّر `directions`؛ (ج) `order_gql.dart` يطلب `directions { lat lng }` في activeOrder+createOrder؛ (د) `OrderModel` (rider) فيه `directions` + getter `routeLine` (المسار إن وُجد وإلا النقاط)؛ (هـ) `aurora_tracking_screen.dart` يرسم `order.routeLine`. الطلبات القديمة تتراجع تلقائياً للنقاط. تحقّق: rider-api tsc=0، rider-app analyze=No issues. **ملاحظة: السعر نفسه كان صحيحاً؛ إن بقي يبدو مرتفعاً فالسبب إعدادات تسعير منطقة قطر في لوحة التحكم لا الكود.**

## 🟢 السبب الجذري الحقيقي لـ«Unauthorized» — استخراج ترويسة المصادقة في Apollo v5 (2026-06-26) — مُصلَح ومنشور ✅
**الاكتشاف الحاسم:** بعد إصلاح العميل + المفاتيح + JWT_SECRET، ظلّ الدخول يفشل (يسجّل خروجاً فوراً). التشخيص النهائي: **الباكند لا يستخرج ترويسة `Authorization`** ⇒ `validate()` لا يُستدعى أصلاً (تأكّد عبر مراقبة Redis: لا استعلام إبطال أثناء `me`). السبب: **Apollo Server v5 / @nestjs/apollo v13** يمرّر ترويسات الطلب أحياناً ككائن **Fetch API `Headers`** (يُقرأ بـ`.get()/.forEach()` لا بالأقواس)، و`buildGraphqlContext` كان يقرأ `headers['authorization']` بالأقواس ⇒ undefined ⇒ التوكن يُسقَط ⇒ **كل طلب مُصادَق يفشل**. هذا هو «Exception: Unauthorized» الأصلي منذ البداية (عطل باكند سابق لكل التغييرات).
- **الإصلاح (PR [#183](https://github.com/3smanco/HANCR/pull/183) + تنظيف [#184](https://github.com/3smanco/HANCR/pull/184)، مدموجان):** `libs/observability/src/lib/graphql-context.ts` — `normalizeHeaders` يدعم كائن Headers (Fetch) + الكائن العادي (Express)، ويطبّع المفاتيح lowercase، ويبحث عن req في كل الأشكال (req/request/extra.request/contextValue.req/raw/المصدر‑نفسه). نُشر (server git pull + `nx build --configuration=production` لـrider+driver + restart).
- **التحقق الحيّ (مُتحقَّق):** راكب `me`→`{id:5,+97433000001}` ✅ · سائق `driverMe`→`{id:3,+97433000010}` ✅ — الدخول المُصادَق يعمل الآن على الباكند.
- **النتيجة:** الهاتف/أرقام التجربة (123456) + Google + الجلسات كلها تعمل الآن (التوكن يُقبل). **التطبيق (النسخة الجديدة) يجب أن يبقى مسجّلاً بعد الدخول.**

## ☁️ ترحيل Google Cloud لحساب جديد (info@zancr.com) — Maps + OAuth (2026-06-25) — جاهز، غير منشور
**السياق:** حساب Google القديم (مشروع `hancr-494520`) فيه بطاقة فوترة مرفوضة عطّلت Maps. المالك جهّز حساباً جديداً فيه رصيد (info@zancr.com) وطلب إنشاء كل شيء فيه.
- **مشروع جديد:** `HANCR` · ID **hancr-500516** · رقم **997014889761** · فوترة "My Billing Account" مربوطة (مدفوع).
- **Maps APIs مُفعّلة:** Maps SDK for Android · Maps JavaScript · Places · Directions · Geocoding.
- **مفتاح Maps الجديد (لكل السطوح — أندرويد/ويب/سيرفر):** `AIzaSyAQ00g8v0BEo4m7V2R8RmoVwJiq-1I5RrQ` (مقيّد بواجهات Maps، بلا قيد تطبيق — مرونة). **بديل سابق (القديم):** android=`...Kk-ws`، web=`...SkZ3o`.
- **OAuth (Google Auth Platform، External، In production):** عميل **Web** `997014889761-64eq7g8i3fnidrdcnhnkpds47ru8r3ap.apps.googleusercontent.com` (يستهلكه: landing GIS + audience الباكند + serverClientId للتطبيقات). شاشة الموافقة منشورة (production) فدخول Google يعمل لكل المستخدمين (نطاقات أساسية، بلا verification).
- **✅ دخول Google داخل التطبيقين مُفعَّل (2026-06-26):** المالك دخل بالحساب القديم؛ **حُذفت كل عملاء OAuth الثلاثة من hancr-494520** (لم يَعُد للحساب القديم أي ارتباط بالمشروع). أُنشئ في hancr-500516 عميلا Android: **HANCR Rider Android** (`997014889761-4q3mb3co48vsr15g0tlltu2g735mieua`) + **HANCR Driver Android** (`997014889761-3973q1oejmhd4n2blgaojhfda2a21qg1`). أُعيد بناء التطبيقين بـ`GOOGLE_SERVER_CLIENT_ID`=عميل Web الجديد (مخبوز في libapp.so، مُتحقَّق) ونُشرا حيّاً. الآن: هاتف/بريد/**Google** تعمل في التطبيقين والموقع.
- **تحديثات الإعدادات (مُنجَزة):** root `.env` (GOOGLE_MAPS_API_KEY + GOOGLE_OAUTH_CLIENT_ID جديدان) · `apps/landing/.env.{local,production}` · **`scripts/deploy-landing.sh`** (يكتب .env.production قاطعاً بالمفاتيح الجديدة — لأن .env* مُتجاهَلة في git؛ هذا المسار الموثوق للسيرفر). Firebase (hancr-88ac0، google-services.json) **لم يُمَس** (FCM مجاني، لا علاقة له بالفوترة).
- **✅ APKs أُعيد بناؤها بالمفتاح الجديد (2026-06-26، arm64):** راكب 50,846,187 + سائق 48,341,400 بايت → `deliverables/hancr-{rider,driver}-20260626.apk`. تحقّق aapt: `com.google.android.geo.API_KEY = AIzaSyAQ00g8v0BEo4m7V2R8RmoVwJiq-1I5RrQ` + `api.hancr.com/{app}/graphql` (production). تجمع: إصلاح Unauthorized + مفتاح Maps الجديد. السائق بلا Google sign-in (`GOOGLE_SERVER_CLIENT_ID` فارغ).
- **⚠️ درس بناء مهم:** مفتاح Maps في المانيفست يأتي من `apps/{app}/android/local.properties` (`GOOGLE_MAPS_API_KEY=`) — أُضيف فيهما (مُتجاهَل في git). الـdart-define وحده لم يحقن المانيفست بثبات. تحقّق المفتاح **بـaapt** لا strings/grep (المانيفست ثنائي). **ذاكرة:** الجهاز **بلا pagefile** (commit limit = RAM فقط) ⇒ AOT يفشل (`malloc failed`) حين يكون كروم مفتوحاً؛ الحل المؤقّت: إغلاق كروم + خفض `gradle.properties` لـ`-Xmx2g/daemon=false/workers.max=1` أثناء البناء ثم `git checkout` لاستعادته (4g). **الدائم (المالك):** تفعيل Automatically manage paging file.
- **✅ نُشر كل شيء حيّاً ومُتحقَّق (2026-06-26):** الـAPKان (النسخة النهائية بدخول Google) على `hancr.com/downloads/hancr-{rider,driver}.apk` (50,846,187 + 48,341,400، HTTP 200، نسخ احتياطية `.bak-20260626`) · backend `/opt/hancr/.env` حُدِّث (GOOGLE_OAUTH_CLIENT_ID + GOOGLE_MAPS_API_KEY الجديدان، نسخة `.env.bak-20260626`) + `pm2 restart rider-api driver-api` (api 200) · الموقع أُعيد بناؤه ونُشر (المفتاحان الجديدان حيّان في `_next`، hancr.com/ar 200). **للمستخدم: ألغِ تثبيت التطبيق القديم قبل تثبيت الجديد.** · **فوترة Google القديمة لم تَعُد مطلوبة.**
- **🟡 ملاحظة بيئة (المالك):** الجهاز المحلي بلا pagefile — فعّل "Automatically manage paging file" (يحتاج صلاحية مدير + إعادة تشغيل) لتفادي فشل بناء APK مستقبلاً.

## 🔐 إصلاح جذر «Exception: Unauthorized» + زر دعم ميت (2026-06-25) — ✅ منشور حيّاً (راكب+سائق، 2026-06-26)
**السياق:** المالك أبلغ (لقطتان) أنه لا يستطيع طلب سائق (`Exception: Unauthorized` في شاشة الطلب)، وأزرار ميتة، وصفحات دعم «تُخرجه من الشاشة». أكّد أن الخطأ **بعد دخول جديد مباشرة** على **APK الإنتاج**. الخطة: `.claude/plans/sorted-yawning-finch.md`.
- **السبب الجذري (مؤكَّد):** `apps/rider-app/lib/core/graphql/graphql_client.dart` كان يخبز التوكن في `HttpLink.defaultHeaders` **وقت بناء العميل مرة واحدة**؛ العميل singleton. بعد الدخول الجديد، استعلامات الطلب (`services`/`routePreview`/`createOrder`) تنطلق على عميل ترويسته بلا التوكن ⇒ حارس JWT يرفض ⇒ Unauthorized. ولا توجد معالجة 401 (لا ErrorLink، لا تحقق صلاحية عند الإقلاع) ⇒ المستخدم محبوس في حالة «داخل لكن مرفوض» فتبدو كل الشاشات معطوبة.
- **الإصلاح (الراكب):**
  - `graphql_client.dart`: **AuthLink** يقرأ التوكن من التخزين الآمن **لكل طلب** (يقضي على الـrace نهائياً) + **ErrorLink** يرصد رسائل `unauthorized/unauthenticated` (GraphQL + ServerException) ويُطلق `onUnauthorized` مرة واحدة (علَم `_authErrorHandled` يُصفَّر في `reset()`). WS لا يزال يأخذ التوكن من `initialPayload` وقت البناء (لذا أُبقي `await reset()` بعد الدخول).
  - `app.dart`: ربط `GraphQLClientManager.onUnauthorized` ببعث `AuthLogoutRequested` (فقط إن كانت الحالة `AuthAuthenticated`) ⇒ الراوتر يوجّه للدخول بدل الحبس.
  - `auth_bloc.dart` `_onCheck`: صار يتحقق من **صلاحية** التوكن عبر استعلام `me` خفيف (يكشف الحظر/التعطيل) بدل فحص وجوده فقط؛ خطأ شبكة فقط ⇒ يبقي الجلسة (أوفلاين متفائل).
- **زر دعم ميت (مطابق للقطة):** زر «Contact us» في `_HelpTopicScreen` (`profile_pages.dart`) كان `onPressed: () => maybePop()` — يُخرج المستخدم دون فعل ⇒ صار `launchSupportEmail(context, subject: title)`. + أُضيف زر «إعادة» لحالة الخطأ في `support_screen.dart`.
- **تدقيق أزرار ميتة (الراكب):** لا معالجات فارغة ولا TODO/UnimplementedError؛ توست «قريباً» للخدمات غير المتاحة سلوك مقصود. لقطة «Account» الفارغة كانت **مقالة مساعدة** (`topicAccountBody`) لا شاشة معطوبة.
- **التحقق:** `flutter analyze` (الراكب كامل) = **No issues found** (74.3s).
- **✅ السائق (مُصلَح أيضاً، نفس العلة):** طُبّق نفس إصلاح **AuthLink + ErrorLink + onUnauthorized** على `driver-app/lib/core/graphql/graphql_client.dart`، وربط `onUnauthorized` في `driver-app/lib/app.dart`، وتحقق صلاحية التوكن عند الإقلاع عبر `driverMeQuery` في `driver-app/lib/blocs/auth/auth_bloc.dart` (يكشف banned/active). `flutter analyze` (الملفات الثلاثة) = **No issues found** (19.1s).
- **✅ بناء APK الإنتاج (2026-06-25، arm64):** الراكب **50,846,187 بايت** + السائق **48,341,400 بايت** (السائق بدون Google sign-in بقرار المالك — هاتف/بريد يعملان). مفتاح Maps أندرويد المستخرَج من الـAPKs السابقة = `AIzaSyDmagwN8nEjMMqtrBuoKxanx00tK7Kk-ws`. تحقّق: كلاهما يحوي `api.hancr.com/{app}/graphql` و **صفر** `10.0.2.2` (ENV=production مخبوز صحيحاً). نُسخا إلى `deliverables/hancr-{rider,driver}-20260625.apk`. (gradle.properties لم يُعدَّل — إصلاح pagefile الدائم كفى.)
- **⚠️ التالي المباشر:** **النشر** على `hancr.com/downloads/hancr-{rider,driver}.apk` (scp لـ`7bici@34.18.212.201` بمفتاح `~/.ssh/google_compute_engine` ثم `sudo cp` لـ`/var/www/hancr-landing/downloads/`) — **بانتظار تأكيد المالك بعد تجربته على هاتفه**. + **إلغاء تثبيت القديم قبل الجديد**. (لا تغييرات backend ⇒ لا إعادة تشغيل APIs.)
- **🟡 تنبيه:** Maps لا تزال معطّلة بسبب فوترة Google ($35.60) — الخريطة قد تظهر رمادية حتى يدفع المالك، لكن **طلب السائق سيعمل** (إصلاح Unauthorized مستقل عن الخرائط).
- **🔴 حواجز المالك (ليست برمجية):** بطاقة فوترة Google Maps المرفوضة (US$35.60) تعطّل الخرائط/التسعير الحي · Twilio تجريبي (OTP لأرقام الاختبار فقط: `+966500000010`←`123456`) · رصيد AI.
