...
- **⏭️ التالي:** C4 (ورقة دفع pre/post-pay — عميقة) · D (إيصال كامل/تقييم محسّن/ما بعده) · F (أمزجة/مزايدة/OTP/ولاء/درع). pickupZones registry + perMinuteWait للعرض الحيّ + خصم رسم الإلغاء فعلياً مؤجَّلة. **فرع `feat/rider-journey-c-to-f` غير مدموج/منشور بعد** (يحتاج migration `1782500000000` عند النشر).
