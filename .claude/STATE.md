...
- **🛑 إعادة بناء APK محظورة بذاكرة الجهاز (لم تُحَل):** 3 محاولات `flutter build apk --release --target-platform android-arm64` (مع `ENV=production`+مفاتيح Maps/Google) فشلت كلها بأخطاء JVM عند بدء Gradle daemon (`G1 virtual space` ثم `Failed to commit metaspace` لـ64KB فقط مع `-Xmx1536m -XX:+UseSerialGC`). `systeminfo`: **Virtual Memory Available تتراجع 786MB→488MB** من Max=16,242MB (يساوي RAM الفعلية تماماً → pagefile شبه معدوم). هذا حدّ commit على مستوى Windows لا علاقة له بـ`-Xmx`. `gradle.properties` أُعيد لحالته الأصلية على main (لم يُعدَّل).
- **⏭️ التالي (يحتاج المالك):** `hancr-driver.apk` المنشور حالياً **لا يزال فيه علّة التعليق + اللوجو القديم** (بُني قبل PR #155/#156/#157). يحتاج: تكبير pagefile على ويندوز أو إغلاق تطبيقات تستهلك commit (عدة claude.exe/chrome.exe/msedge) → إعادة بناء arm64 (recipe السابق `-Xmx2g`+`daemon=false`+`parallel=false`+`workers.max=1`) → نشر لـ`hancr.com/downloads/hancr-driver.apk`.
