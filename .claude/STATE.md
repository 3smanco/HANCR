...
- **⏭️ التالي:** C4 (ورقة دفع + tفصيل أجرة) → D (إيصال كامل/تقييم محسّن/ما بعده) → E (إلغاء+رسوم) → F (أمزجة/مزايدة/OTP/ولاء/درع). **لم يُدمج/يُنشر — كود محلي** (يحتاج migration `1782400000000` على السيرفر + بناء APK). pickupZones registry + perMinuteWait للعرض الحيّ مؤجَّلان.
