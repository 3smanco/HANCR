...
- تقوية فحص الجاهزية حتى يرفض raw IP في الروابط العامة وCORS حتى لو كان
  الرابط HTTPS؛ الإنتاج يجب أن يستخدم DNS حقيقي.
- جعل `npm run ci:verify` يجلب اعتماديات Flutter لتطبيقي الراكب والسائق قبل
  `flutter analyze --no-pub` حتى لا يعتمد على حالة `.dart_tool` المحلية.
- إضافة اختبار يحمي سلوك الرحلات المشتركة: لا يعود السائق إلى `Online` إذا
  بقي له طلب آخر في حالة `DriverAccepted` أو `Arrived` أو `Started`.
- تقوية لوحة الإدارة:
  - lint بدون warnings.
  - فحص مفاتيح الترجمة.
...
- تقليل تسريب بيانات الاتصال في logs:
  ...
  - عدم طباعة OTP نفسه في سجلات auth حتى في وضع dev.
...
- Sentry لتطبيقات Flutter مفعّل في الكود، ويحتاج فقط تمرير
  `--dart-define=SENTRY_DSN=...` عند بناء نسخ الإنتاج حتى يبدأ إرسال crash reports.
...
- تشغيل `npm run ci:verify` كشرط للـ PR، بما يشمل فحص redaction للسجلات.
