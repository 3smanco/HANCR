...
In development and CI, Nest GraphQL writes `apps/*/schema.gql`; commit those
files whenever the API shape changes. In production, the APIs use in-memory
schema generation so PM2 startup and reloads do not rewrite tracked schema
files on the server. CI still checks that generated schema files do not stay
dirty after tests.
