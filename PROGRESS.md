...
## ✅ Phase 11 — ترجمة كاملة لشاشات الراكب
- أُعيدت هيكلة `app_localization.dart` لبنية **مفتاح-أولاً** (كل مفتاح + 8 لغات). ~160 مفتاحاً.
- تُرجمت كل الشاشات النشطة الأساسية: المصادقة (هاتف/OTP)، تبويب الخدمات، الحجز، النشاط+تفاصيل الرحلة، الحساب، المحفظة+شحن، التتبع، التقييم، الولاء، SOS+جهات الطوارئ+إضافة جهة، مركز المساعدة+FAQ، الإشعارات، العروض، الإعدادات.
- استخدام `tr('key')` في كل مكان؛ معالجة const (تحويل `const` widgets الحاوية لـ tr).
- **متبقٍ (ثانوي):** نصوص في شاشات قديمة مكرّرة (غير معروضة)، رموز الأعلام، وبعض نصوص SOS التفاعلية المتداخلة (interpolated).
- 0 أخطاء ترجمة. APK منشور.
...
### 8.3 — جلسة مستقلة: تحقق + تثبيت نهائي
- محاولة تحقق على المحاكي: **فشلت بيئياً** — المحاكي (2GB) يدخل دوامة `lowmemorykiller` ويقتل التطبيق قبل رسم أول إطار (الجهاز 16GB مع تطبيقات كثيرة). أُكّد أنه قيد بيئي لا خطأ تطبيق ("flutter loaded normally" ثم قتل OOM).
- تأكيدات لا تحتاج جهاز: Maps SDK Android **مُفعّل**، الفوترة **مُفعّلة** (`billingEnabled: true`)، المفتاح الجديد مُضمّن داخل الـ APKs (تحقق aapt).
- لون إشعار violet→ember للاتساق.
- **إعادة اختبار E2E على الإنتاج:** طلب→قبول→وصول→بدء→إنهاء ✅ (Order #4).
- صحة سطح المعاينة: hancr.com 200 · admin 307→login · api 200 · كلا الـ APK 200 ✅.
- APKs نهائية متطابقة مع المصدر منشورة.

**بانتظار:** اختبار المستخدم للإصلاحين على جهاز فعلي (الراكب: هاتف→OTP→رئيسية؛ السائق: الخريطة تظهر).
